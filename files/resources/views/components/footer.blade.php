        ...
        <div class="mt-4 flex flex-wrap items-baseline justify-between gap-x-8 gap-y-2">
            ...
            <ul role="list" class="-mx-2 flex flex-wrap items-baseline gap-x-1">
                @foreach ($site->social as $item)
                    <li>
                        <a href="{{ $item->url }}" target="_blank" rel="noopener" class="rounded-md px-2 py-1 text-base text-muted hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent sm:text-[0.9375rem]">{{ $item->label }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
