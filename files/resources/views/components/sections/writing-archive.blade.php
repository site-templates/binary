        ...
        @foreach ($writing as $entry)
            <li>
                <a href="{{ $entry->link }}" class="row flex flex-col gap-x-8 gap-y-1 py-2.5 md:flex-row md:items-baseline">
                    <span class="order-2 shrink-0 font-mono text-sm text-muted tabular-nums md:order-1 md:w-28">{{ $entry->dateFormatted }}</span>
                    <span class="order-1 min-w-0 md:order-2 md:flex-1">
                        <span class="flex items-start gap-1.5 text-base sm:text-[0.9375rem]">
                            <span class="min-w-0 text-pretty">{{ $entry->title }}</span>
